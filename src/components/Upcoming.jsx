...
const GreenLeaveEvents = () => {
  ...
  useEffect(() => {
    let scene, camera, renderer;
    let voiceStarted = false; // Track if the voice sequence has started
    ...
    const init = () => {
      ...
      const unlockVoice = () => {
        ...
        if (!voiceStarted) {
          voiceStarted = true;
          showStep(0); // Start the steps even before AR starts
        }
      };
      document.addEventListener("click", unlockVoice, { once: true });
      ...
      // Show steps (voice + 3D if AR active)
      function showStep(index) {
        ...
        // 🧊 Only show model if AR is active
        if (isARActive) {
          scene.children
            .filter((child) => !(child instanceof THREE.Light))
            .forEach((child) => scene.remove(child));
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden">
      ...
      {/* Step overlay (always visible while sequence runs) */}
      <div
        ref={overlayRef}
        className="fixed top-8 left-1/2 -translate-x-1/2 bg-blue-600 text-white px-8 py-4 rounded-2xl text-center max-w-md shadow-2xl border-4 border-white"
        style={{ zIndex: 9999 }}
      >
        <p className="text-2xl font-bold">
          {steps[currentStep]?.text || "Preparing instructions..."}
        </p>
      </div>
      ...
      {!isARActive && (
        <div className="fixed inset-0 flex flex-col items-center justify-center pointer-events-none">
          ...
          <div className="bg-blue-600 text-white text-center px-6 py-3 rounded-lg shadow-lg max-w-xs mx-4 relative -mt-8">
            <p className="text-base font-medium">
              Tap anywhere to enable sound, then tap <strong>“Start AR”</strong> below to view models.
            </p>
          </div>
        </div>
      )}
